...
type ColumnType = ColumnDef<ConsultationInterface, unknown>;
...
export function ConsultationListPage(props: ConsultationListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ConsultationInterface>>(
    () => `/consultations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ConsultationInterface) => {
    if (hasAccess('consultation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/consultations/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/consultations/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="consultation" />
